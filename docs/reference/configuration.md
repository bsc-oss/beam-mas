...
### PG Session Expiration

This feature automatically expires OAuth2 sessions based on their scope:

- Sessions with `urn:pg:session:short-lived` scope expire after `short_lived_max_lifetime` (default: 10 hours)
- Sessions with `urn:pg:session:long-lived` scope expire after `long_lived_max_lifetime` (default: 30 days)
- Sessions without either scope are **not affected** by this feature
- Sessions must have been inactive for at least `inactivity_ttl` (default: 30 minutes) before being expired

The job runs every 15 minutes to check for sessions that need to be expired.
...
pg:
  session_expiration:
    ...
    # Value is in seconds. Defaults to 36000 (10 hours).
    short_lived_max_lifetime: 36000

    # Maximum lifetime for sessions with the `urn:pg:session:long-lived` scope.
    # Sessions older than this will be expired if they have been inactive for
    # at least `inactivity_ttl`.
    # Value is in seconds. Defaults to 2592000 (30 days).
    long_lived_max_lifetime: 2592000
    ...
    # Value is in seconds. Defaults to 1800 (30 minutes).
    inactivity_ttl: 1800
```

**Minimal configuration** (uses all defaults, just enables the feature):
...
The following PG-specific scopes are available:

| Scope | Description |
|-------|-------------|
| `urn:pg:mas-logout` | When a session with this scope revokes its access or refresh token, MAS will also end the browser session and all other OAuth2 sessions linked to it. |
| `urn:pg:session:short-lived` | Sessions with this scope are automatically expired after `short_lived_max_lifetime` (default: 10 hours) if inactive. |
| `urn:pg:session:long-lived` | Sessions with this scope are automatically expired after `long_lived_max_lifetime` (default: 30 days) if inactive. |
