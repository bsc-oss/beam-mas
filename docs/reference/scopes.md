...
The [default policy](../topics/policy.md#authorization-requests) shipped with MAS supports the following scopes:
 ...
 - [`urn:pg:mas-logout`](#urnpgmas-logout)
 - [`urn:pg:session:short-lived`](#urnpgsessionshort-lived)
 - [`urn:pg:session:long-lived`](#urnpgsessionlong-lived)
...
These scopes are custom extensions specific to this fork.

### `urn:pg:mas-logout`

This scope enables full logout behaviour when a client revokes their token.
When a session with this scope revokes its access or refresh token, MAS will also end the browser session (MAS session) that the OAuth2 session is linked to, along with all other OAuth2 sessions linked to that browser session.

This necessary to ensure the user is fully logged out of MAS when they log out of the application.

The default policy allows any client and any user to request this scope for interactive grants (authorization code and device authorization grants).

### `urn:pg:session:short-lived`

This scope indicates that the session should have a short lifetime. Default: 10 hours + 30 mins of inactivity

The default policy allows any client and any user to request this scope for interactive grants.

### `urn:pg:session:long-lived`

This scope indicates that the session should have a long lifetime. Default: 30 days + 30 mins of inactivity
