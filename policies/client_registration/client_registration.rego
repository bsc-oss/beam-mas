...
# Normalize URI by removing trailing slash for comparison
normalize_uri(uri) := normalized if {
    is_string(uri)
    endswith(uri, "/")
    normalized := substring(uri, 0, count(uri) - 1)
}

normalize_uri(uri) := uri if {
    is_string(uri)
    not endswith(uri, "/")
}

client_uri_in_whitelist if {
    some allowed_uri in data.client_registration.allowed_client_uris
    normalize_uri(input.client_metadata.client_uri) == normalize_uri(allowed_uri)
}

violation contains {"msg": "client_uri not in allowed list"} if {
    # Check if we have a whitelist configured
    count(data.client_registration.allowed_client_uris) > 0
    # Check if the client_uri is not in the whitelist
    not client_uri_in_whitelist
}
