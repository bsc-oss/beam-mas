...
# PG_CHANGED
# This scope allows the client to log out the user from MAS when revoking the token
allowed_scope("urn:pg:mas-logout") if {
	interactive_grant_type(input.grant_type)
}

# Session lifetime scopes
allowed_scope("urn:pg:session:short-lived") if {
	interactive_grant_type(input.grant_type)
}

allowed_scope("urn:pg:session:long-lived") if {
	interactive_grant_type(input.grant_type)
}
