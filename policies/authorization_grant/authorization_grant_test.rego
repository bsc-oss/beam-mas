...
# PG_CHANGED
test_pg_mas_logout_scope if {
	# urn:pg:mas-logout scope should be allowed for interactive grants
	authorization_grant.allow with input.user as user
		with input.client as client
		with input.grant_type as "authorization_code"
		with input.scope as "urn:pg:mas-logout"
	...
	# urn:pg:mas-logout scope should not be allowed for client_credentials grant
	not authorization_grant.allow with input.user as user
		with input.client as client
		with input.grant_type as "client_credentials"
		with input.scope as "urn:pg:mas-logout"
...
test_pg_session_lifetime_scopes if {
	# Session lifetime scopes should be allowed for interactive grants
	authorization_grant.allow with input.user as user
		with input.client as client
		with input.grant_type as "authorization_code"
		with input.scope as "urn:pg:session:short-lived"

	authorization_grant.allow with input.user as user
		with input.client as client
		with input.grant_type as "authorization_code"
		with input.scope as "urn:pg:session:long-lived"

	# Should not be allowed for client_credentials grant
	not authorization_grant.allow with input.user as user
		with input.client as client
		with input.grant_type as "client_credentials"
		with input.scope as "urn:pg:session:short-lived"
