...
use mas_config::{
    ...
    // PG_CHANGED: expiring-sesssions
    PgConfig, PolicyConfig, TemplatesConfig,
};
use mas_context::LogContext;
use mas_data_model::{PgSessionExpirationConfig, SessionExpirationConfig, SessionLimitConfig, SiteConfig};
...
pub fn site_config_from_config(
    ...
    // PG_CHANGED: expiring-sesssions
    let pg_session_expiration = pg_config.session_expiration.as_ref().map(|c| {
        PgSessionExpirationConfig {
            enabled: c.enabled,
            short_lived_max_lifetime: c.short_lived_max_lifetime,
            long_lived_max_lifetime: c.long_lived_max_lifetime,
            inactivity_ttl: c.inactivity_ttl,
        }
    });
