...
impl Filter for OAuth2SessionFilter<'_> {
    fn generate_condition(&self, _has_joins: bool) -> impl sea_query::IntoCondition {
        sea_query::Condition::all()
            ...
            // PG_CHANGED: filter on any of the scopes (OR)
            .add_option(self.scope_any().map(|scopes| {
                // OR filter: session contains any of the given scopes
                scopes
                    .iter()
                    .map(|s| {
                        Expr::col((OAuth2Sessions::Table, OAuth2Sessions::ScopeList))
                            .contains(vec![(*s).to_owned()])
                    })
                    .reduce(|a, b| a.or(b))
                    .expect("scope_any should have at least one scope")
            }))
            ...
            // PG_CHANGED: filter on created_at
