...
impl Translator {
    ...
    pub fn plural_with_fallback(
        ...
        loop {
            ...
            // PG_CHANGED - adds fallback to default locale for plurals
            // Try the default locale if we hit the `und` locale
            if locale.is_und() {
                let message = self.plural(&self.default_locale, key, count).ok()?;
                return Some((message, self.default_locale.clone()));
            }
...
mod tests {
    ...
    #[test]
    fn test_plural_fallback_to_default_locale() {
        let translator = translator();

        // Request a plural key that only exists in English ("en") but not in French ("fr")
        // This should fall back to the default locale (English) instead of returning None
        let result = translator.plural_with_fallback(locale!("fr").into(), "english_only_plural", 1);
        assert!(
            result.is_some(),
            "Should fall back to default locale when plural key is missing"
        );

        let (message, locale) = result.unwrap();
        let formatted = message.format(&arg_list!(count = 1)).unwrap();
        assert_eq!(formatted, "1 item.");
        assert_eq!(locale, locale!("en").into());

        // Also test with count > 1
        let (message, locale) = translator
            .plural_with_fallback(locale!("fr").into(), "english_only_plural", 5)
            .unwrap();
        let formatted = message.format(&arg_list!(count = 5)).unwrap();
