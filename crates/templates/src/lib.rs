...
mod tests {
    ...
    #[test]
    fn test_pg_display_user_identity_macro() {
        let mut env = minijinja::Environment::new();

        // Load the macro template
        env.add_template(
            "components/pg_display_user.html",
            include_str!("../../../templates/components/pg_display_user.html"),
        )
        .unwrap();
        ...
        env.add_template(
            "test.html",
            r#"{% import "components/pg_display_user.html" as pg_display_user %}{{ pg_display_user.identity(display_name, email, username) }}"#,
        )
        .unwrap();

        let tmpl = env.get_template("test.html").unwrap();

        // Test case 1: Both display_name and email are defined
        let result = tmpl
            ...
            .unwrap();
        assert_eq!(result.trim(), "John Doe (john@example.com)");

        // Test case 2: Only email is defined
        let result = tmpl
            .render(minijinja::context! {
                display_name => "",
                email => "john@example.com",
            ...
            .unwrap();
        assert_eq!(result.trim(), "john@example.com");

        // Test case 3: Only email is defined (display_name is None)
        let result = tmpl
            .render(minijinja::context! {
                email => "john@example.com",
                username => "johndoe"
        ...
        // Test case 4: Only username (fallback)
        let result = tmpl
            .render(minijinja::context! {
                display_name => "",
                email => "",
            ...
            .unwrap();
        assert_eq!(result.trim(), "johndoe");

        // Test case 5: Only username (both None)
        let result = tmpl
            .render(minijinja::context! {
                username => "johndoe"
            })
