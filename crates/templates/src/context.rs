...
impl IndexContext {
    ...
    // PG_CHANGED
    /// Set the display name and email for the index page
    #[must_use]
    pub fn with_display_name_and_email(
        mut self,
        display_name: Option<String>,
        email: Option<String>,
    ) -> Self {
...
impl AppContext {
    ...
    // PG_CHANGED
    /// Attach a CSRF token that will be passed to the frontend app config
    #[must_use]
    pub fn with_csrf_token(mut self, token: impl Into<String>) -> Self {
        self.app_config.csrf_token = Some(token.into());
        self
    }
}
...
impl AccountInactiveContext {
    ...
    /// PG_CHANGED: post-logout action Sets the post-auth action to redirect to after logout
    #[must_use]
    pub fn with_action(mut self, action: PostAuthAction) -> Self {
...
impl TemplateContext for AccountInactiveContext {
        ...
        Self: Sized,
    {
        sample_list(
            User::samples(now, rng)
                .into_iter()
                .map(|user| AccountInactiveContext { user, action: None })
                .collect(),
        )
    }
}
