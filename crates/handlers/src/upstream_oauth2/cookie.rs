...
/// PG_CHANGED: Sessions expire after 20 minutes instead of 10
static SESSION_MAX_TIME: Duration = Duration::microseconds(20 * 60 * 1000 * 1000);
...
mod tests {
    ...
    fn test_session_cookie() {
        ...
        // PG_CHANGED: SESSION_MAX_TIME is 20 minutes, so advance 16 more minutes
        // (5 + 16 = 21 minutes total from first session creation)
        let now = now + Duration::microseconds(16 * 60 * 1000 * 1000);
        let sessions = sessions.expire(now);
