...
pub enum RouteError {
    ...
    // PG_CHANGED
    #[error("Mutations and subscriptions cannot be executed via GET; retry the request as POST")]
    OperationNotAllowed,
    // PG_CHANGED
    #[error("Failed to parse GraphQL document: {0}")]
...
pub async fn post(
    ...
    // PG_CHANGED
    if requester.browser_session().is_some() {
        let header_value = headers
            .get(CSRF_HEADER)
            .ok_or(CsrfError::MissingToken)?
            .to_str()
            .map_err(|_| CsrfError::Mismatch)?;
        cookie_jar = cookie_jar.verify_token_value(&clock, header_value)?;
    }
...
// PG_CHANGED
fn pg_ensure_get_only_allows_queries(request: &async_graphql::Request) -> Result<(), RouteError> {
    let document = parse_query(&request.query)
        .map_err(|err| RouteError::ParseDocument(err.to_string()))?;
    let operation_type = if let Some(target) = request.operation_name.as_deref() {
        document.operations.iter().find_map(|(name, operation)| {
            name.and_then(|name| {
    ...
    match operation_type {
        Some(OperationType::Mutation | OperationType::Subscription) => {
            Err(RouteError::OperationNotAllowed)
        }
        _ => Ok(()),
    }
}
...
pub async fn get(
    ...
    // PG_CHANGED
    pg_ensure_get_only_allows_queries(&request)?;
...
pub fn schema_builder() -> SchemaBuilder {
    async_graphql::Schema::build(Query::new(), Mutation::new(), EmptySubscription)
        .disable_introspection() // PG_CHANGED: Disable introspection
