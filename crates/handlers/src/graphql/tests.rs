...
// PG_CHANGED
async fn browser_session_with_csrf(
    state: &TestState,
    username: &str,
) -> (CookieHelper, String) {
    ...
    let cookie_jar = state.cookie_manager.cookie_jar().set_session(&session);
    let (csrf_token, cookie_jar) = cookie_jar.csrf_token(&*state.clock, &mut rng);

    let helper = CookieHelper::new();
    helper.import(cookie_jar);

    (helper, csrf_token.form_value())
}
...
// PG_CHANGED
/// Test that GET requests cannot perform mutations.
#[sqlx::test(migrator = "mas_storage_pg::MIGRATOR")]
async fn test_get_rejects_mutation(pool: PgPool) {
    setup();
    let state = TestState::from_pool(pool).await.unwrap();

    let request = Request::get(
        "/graphql?query=mutation%20{deactivateUser(input:%20{hsErase:%20true,%20password:%20null})%20{status}}",
    )
    .empty();
    ...
    assert_eq!(
        ...
        vec![serde_json::json!({
            "message":
                "Only query operations may be executed over GET; use POST for other operations",
        })],
    );
...
// PG_CHANGED
/// Test that browser sessions must supply the CSRF header on POST.
#[sqlx::test(migrator = "mas_storage_pg::MIGRATOR")]
async fn test_browser_session_requires_csrf(pool: PgPool) {
    setup();
    let state = TestState::from_pool(pool).await.unwrap();

    let (cookies, csrf_token) = browser_session_with_csrf(&state, "charlie").await;

    let request = Request::post("/graphql").json(serde_json::json!({
        "query": r#"
            mutation {
                deactivateUser(input: { hsErase: true, password: null }) {
                    status
                }
            }
        "#,
    ...
    let response = state.request(cookies.clone().with_cookies(request.clone())).await;
    response.assert_status(StatusCode::FORBIDDEN);

    // Supplying header should succeed (GraphQL still returns meaningful response)
    let request = cookies
        .with_cookies(
...
async fn test_set_password_rejected_wrong_password(pool: PgPool) {
    ...
    let request = Request::post("/graphql")
        // PG_CHANGED
        .header("X-CSRF-Token", csrf_token.form_value())
        .json(serde_json::json!({
        "query": format!(r#"
                mutation {{
                    setPassword(input: {{
