...
// PG_CHANGED: pg_scopes
use mas_data_model::{BoxClock, BoxRng, TokenType, pg_scopes};
use mas_iana::oauth::OAuthTokenTypeHint;
use mas_keystore::Encrypter;
// PG_CHANGED: auth2::{OAuth2SessionFilter, OAuth2SessionRepository}, user::BrowserSessionRepository,
use mas_storage::{
...
pub(crate) async fn post(
    ...
    // PG_CHANGED: Store the browser session ID and scope before consuming the OAuth2 session
    let user_session_id = session.user_session_id;
    let has_mas_logout_scope = session.scope.contains(&pg_scopes::PG_MAS_LOGOUT);

    // Now that we checked everything, we can end the session.
    repo.oauth2_session().finish(&clock, session).await?;

    // If the session has the urn:pg:mas-logout scope, also finish the linked browser session (MAS session)
    // and all other OAuth2 sessions linked to that browser session
    if let (true, Some(user_session_id)) = (has_mas_logout_scope, user_session_id) {
        let browser_session = repo.browser_session().lookup(user_session_id).await?;
        if let Some(browser_session) = browser_session.filter(|s| s.finished_at.is_none()) {
            // Finish all other OAuth2 sessions linked to this browser session
            ...
            // Finish the browser session itself
            repo.browser_session().finish(&clock, browser_session).await?;

            tracing::info!(
                browser_session.id = %user_session_id,
                oauth2_sessions_affected,
                "PG_LOG: Finished linked browser session and OAuth2 sessions during OAuth2 token revocation with urn:pg:mas-logout scope"
            );
        }
    }
...
mod tests {
    ...
    /// PG_CHANGED: Test that revoking a token with the urn:pg:mas-logout scope
    /// also finishes the browser session and all linked OAuth2 sessions.
    #[sqlx::test(migrator = "mas_storage_pg::MIGRATOR")]
    async fn test_revoke_with_mas_logout_scope(pool: PgPool) {
        setup();
        let state = TestState::from_pool(pool).await.unwrap();
        ...
        let browser_session_id = browser_session.id;
        ...
        // Create first OAuth2 session WITH the mas-logout scope
        let session_with_logout_scope = repo
            .oauth2_session()
            .add_from_browser_session(
                &mut state.rng(),
                &state.clock,
                &client,
                &browser_session,
                Scope::from_iter([OPENID, pg_scopes::PG_MAS_LOGOUT]),
            )
            .await
            .unwrap();

        // Create second OAuth2 session WITHOUT the mas-logout scope (linked to same browser session)
        let session_without_logout_scope = repo
            .oauth2_session()
            .add_from_browser_session(
                &mut state.rng(),
                &state.clock,
        ...
        let (AccessToken { access_token: access_token_with_logout, .. }, RefreshToken { refresh_token: refresh_token_with_logout, .. }) =
            generate_token_pair(
                &mut state.rng(),
                &state.clock,
                &mut repo,
                &session_with_logout_scope,
                Duration::microseconds(5 * 60 * 1000 * 1000),
            )
            .await
            .unwrap();

        let (AccessToken { access_token: access_token_without_logout, .. }, _) =
            generate_token_pair(
                &mut state.rng(),
                &state.clock,
                &mut repo,
                &session_without_logout_scope,
                Duration::microseconds(5 * 60 * 1000 * 1000),
            )
            .await
            .unwrap();
        ...
        assert!(state.is_access_token_valid(&access_token_with_logout).await);
        assert!(state.is_access_token_valid(&access_token_without_logout).await);

        // Verify browser session is active
        let mut repo = state.repository().await.unwrap();
        let browser_session_before = repo.browser_session().lookup(browser_session_id).await.unwrap().unwrap();
        assert!(browser_session_before.finished_at.is_none(), "Browser session should be active before revocation");
        drop(repo);

        // Revoke the token that has the mas-logout scope
        let request = Request::post(mas_router::OAuth2Revocation::PATH).form(serde_json::json!({
            "token": refresh_token_with_logout,
            "token_type_hint": "refresh_token",
            "client_id": client_id,
            "client_secret": client_secret,
        }));
        ...
        // Both access tokens should now be invalid (both sessions were terminated)
        assert!(!state.is_access_token_valid(&access_token_with_logout).await);
        assert!(!state.is_access_token_valid(&access_token_without_logout).await);

        // Browser session should be finished
        let mut repo = state.repository().await.unwrap();
        let browser_session_after = repo.browser_session().lookup(browser_session_id).await.unwrap().unwrap();
        assert!(browser_session_after.finished_at.is_some(), "Browser session should be finished after revocation with mas-logout scope");
    }

    /// PG_CHANGED: Test that revoking a token WITHOUT the urn:pg:mas-logout scope
    /// does NOT finish the browser session or other linked OAuth2 sessions.
    #[sqlx::test(migrator = "mas_storage_pg::MIGRATOR")]
    async fn test_revoke_without_mas_logout_scope_does_not_affect_browser_session(pool: PgPool) {
        setup();
        let state = TestState::from_pool(pool).await.unwrap();
        ...
        // Create first OAuth2 session WITHOUT the mas-logout scope
        let session_to_revoke = repo
            .oauth2_session()
            .add_from_browser_session(
                &mut state.rng(),
                &state.clock,
        ...
        // Create second OAuth2 session (also without mas-logout scope, linked to same browser session)
        ...
        // Generate tokens for both sessions
        let (AccessToken { access_token: access_token_to_revoke, .. }, RefreshToken { refresh_token: refresh_token_to_revoke, .. }) =
            generate_token_pair(
                &mut state.rng(),
                &state.clock,
                &mut repo,
        ...
        let (AccessToken { access_token: other_access_token, .. }, _) =
            generate_token_pair(
                &mut state.rng(),
                &state.clock,
                &mut repo,
        ...
        // Verify both tokens are valid
        assert!(state.is_access_token_valid(&access_token_to_revoke).await);
        assert!(state.is_access_token_valid(&other_access_token).await);

        // Revoke the token WITHOUT the mas-logout scope
        let request = Request::post(mas_router::OAuth2Revocation::PATH).form(serde_json::json!({
        ...
        // Only the revoked token should be invalid
        assert!(!state.is_access_token_valid(&access_token_to_revoke).await);
        // The other session should still be valid
        assert!(state.is_access_token_valid(&other_access_token).await);

        // Browser session should still be active
        let mut repo = state.repository().await.unwrap();
        let browser_session_after = repo.browser_session().lookup(browser_session_id).await.unwrap().unwrap();
        assert!(browser_session_after.finished_at.is_none(), "Browser session should still be active after revocation without mas-logout scope");
    }
}
