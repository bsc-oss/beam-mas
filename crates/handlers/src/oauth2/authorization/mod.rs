    ...
    let res: Result<Response, RouteError> = ({
        ...
        async move {
            ...
            let code: Option<AuthorizationCode> = if response_type.has_code() {
                ...
                // PG_CHANGED: PKCE is required for authorization code flow
                let Some(pkce_params) = params.pkce else {
                    return Ok(callback_destination.go(
                        &templates,
                        &locale,
                        ClientError::from(ClientErrorCode::InvalidRequest)
                            .with_description("PKCE is required. Please provide code_challenge and code_challenge_method parameters.".to_owned()),
                    )?);
                };
                ...
                // PG_CHANGED
                let pkce = Pkce {
                    challenge: pkce_params.code_challenge,
                    challenge_method: pkce_params.code_challenge_method,
                };

                Some(AuthorizationCode { code, pkce: Some(pkce) })
            } else {
