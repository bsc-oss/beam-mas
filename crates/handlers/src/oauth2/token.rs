...
async fn authorization_code_grant(
    ...
    match (code.pkce.as_ref(), grant.code_verifier.as_ref()) {
        // PG_CHANGED: PKCE is required: reject requests without a code_challenge
        (None, None) | (None, Some(_)) => {
            debug!("PKCE is required but no code_challenge was provided");
            return Err(RouteError::BadRequest);
        }
        // We have a challenge but no verifier? Bad request.
        (Some(_), None) => return Err(RouteError::BadRequest),
    ...
    if !refresh_token.is_valid() {
        ...
        if !next_refresh_token.is_valid() {
            ...
            warn!(
                ...
                original_refresh_token.id = %refresh_token.id,
                reissued_refresh_token.id = %next_refresh_token.id,
                "CSIRT ALERT: probably replay attack - refresh-token re-used."
            );
        ...
        warn!(
            ...
            original_refresh_token.id = %refresh_token.id,
            reissued_refresh_token.id = %next_refresh_token.id,
            "CSIRT ALERT: refresh-token re-used - issued refresh and access tokens are unused. Assuming those were lost."
        );
...
mod tests {
    use hyper::Request;
    // PG_CHANGED
    use mas_data_model::{AccessToken, AuthorizationCode, Pkce, RefreshToken};
    ...
    // PG_CHANGED: Test code verifier and challenge from RFC7636 appendices
    ...
    // PG_CHANGED
    #[sqlx::test(migrator = "mas_storage_pg::MIGRATOR")]
    async fn test_auth_code_grant_without_pkce_is_rejected(pool: PgPool) {
        setup();
        let state = TestState::from_pool(pool).await.unwrap();
        ...
        // Create a grant WITHOUT PKCE (pkce: None)
        let code = "codenopkce";
        let grant = repo
            .oauth2_authorization_grant()
            .add(
        ...
        // Try to exchange without PKCE - should be rejected because PKCE is required
        let request =
            Request::post(mas_router::OAuth2TokenEndpoint::PATH).form(serde_json::json!({
                "grant_type": "authorization_code",
                "code": code,
        ...
        response.assert_status(StatusCode::BAD_REQUEST);
        let ClientError { error, .. } = response.json();
        assert_eq!(error, ClientErrorCode::InvalidRequest);
    }
