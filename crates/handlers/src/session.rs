...
/// PG_CHANGED: post-logout action
/// If `post_auth_action` is provided, it will be included in the fallback pages
/// so that the user can be redirected back to the original flow after logging
/// in again.
pub async fn load_session_or_fallback(
    ...
    if session.finished_at.is_some() {
        ...
        let (csrf_token, cookie_jar) = cookie_jar.csrf_token(clock, rng);
         // PG_CHANGED: post-logout action
        let mut ctx = AccountInactiveContext::new(session.user);
        if let Some(action) = post_auth_action {
            ctx = ctx.with_action(action);
        }
        let ctx = ctx
            .with_csrf(csrf_token.form_value())
            .with_language(locale.clone());
