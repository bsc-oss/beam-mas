...
pub struct Provider {
    ...
    // PG_CHANGED
    /// The app scheme to use for deep linking after device code consent.
    /// When set, users will be redirected to `{pg_app_scheme}:/qr-login-consent`
    /// after giving consent for a device code grant.
    ///
    /// Example: `be.devserver.local` will redirect to `be.devserver.local:/qr-login-consent`
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pg_app_scheme: Option<String>,
