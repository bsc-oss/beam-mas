...
/// PG_CHANGED Scheduled job to expire OAuth2 sessions based on their scope lifetime
///
/// This job will trigger the [`ExpirePgOAuthSessionsJob`] for each scope.
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct ExpirePgSessionsJob;

impl InsertableJob for ExpirePgSessionsJob {
    const QUEUE_NAME: &'static str = "expire-pg-sessions";
}

/// Expire OAuth2 sessions based on their scope lifetime
///
/// Sessions with `urn:pg:session:short-lived` or `urn:pg:session:long-lived`
/// scopes will be expired if they exceed their maximum lifetime and have been
/// inactive for at least the configured inactivity TTL.
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct ExpirePgOAuthSessionsJob {
    /// Sessions created before this time with the short-lived scope will be
    /// expired
    short_lived_created_before: DateTime<Utc>,
    /// Sessions created before this time with the long-lived scope will be
    /// expired
    long_lived_created_before: DateTime<Utc>,
    /// Sessions must have been inactive since before this time to be expired
    inactive_before: DateTime<Utc>,
    /// Pagination cursor for batching
    after: Option<Ulid>,
}

impl ExpirePgOAuthSessionsJob {
    /// Create a new job to expire OAuth2 sessions based on their scope lifetime
    ///
    /// # Parameters
    ///
    /// * `short_lived_created_before` - Sessions with short-lived scope created
    ///   before this time will be expired
    /// * `long_lived_created_before` - Sessions with long-lived scope created
    ///   before this time will be expired
    /// * `inactive_before` - Sessions must have been inactive since before this
    ///   time
    #[must_use]
    pub fn new(
        short_lived_created_before: DateTime<Utc>,
    ...
    /// Get the threshold for short-lived sessions
    #[must_use]
    pub fn short_lived_created_before(&self) -> DateTime<Utc> {
        self.short_lived_created_before
    }

    /// Get the threshold for long-lived sessions
    #[must_use]
    pub fn long_lived_created_before(&self) -> DateTime<Utc> {
        self.long_lived_created_before
    }

    /// Get the inactivity threshold
    #[must_use]
    pub fn inactive_before(&self) -> DateTime<Utc> {
    ...
    pub fn next(&self, page: &Page<Session>) -> Option<Self> {
        ...
        let last_edge = page.edges.last()?;
        Some(Self {
            short_lived_created_before: self.short_lived_created_before,
            long_lived_created_before: self.long_lived_created_before,
            inactive_before: self.inactive_before,
            after: Some(last_edge.cursor),
        })
    }
}

impl InsertableJob for ExpirePgOAuthSessionsJob {
    const QUEUE_NAME: &'static str = "expire-pg-oauth-sessions";
}
