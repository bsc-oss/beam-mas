...
impl<'a> OAuth2SessionFilter<'a> {
    ...
    /// PG_CHANGED: Only return sessions created before the given time
    #[must_use]
    ...
    /// PG_CHANGED: Get the created before filter
    ///
    ...
    /// PG_CHANGED: Only return sessions that contain any of the given scope strings
    ///
    /// This is an OR filter - sessions matching any one of the scopes will be
    /// returned.
    #[must_use]
    pub fn with_scope_containing_any(mut self, scopes: &'a [&'a str]) -> Self {
        self.scope_any = Some(scopes);
        self
    }

    /// PG_CHANGED: Get the scope_any filter
    ///
    /// Returns [`None`] if no scope_any filter was set
    #[must_use]
    pub fn scope_any(&self) -> Option<&'a [&'a str]> {
        self.scope_any
    }
