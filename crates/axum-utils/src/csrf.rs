...
pub enum CsrfError {
    ...
    // PG_CHANGED
    /// The token was not provided in the request
    #[error("Missing CSRF token value")]
    MissingToken,
...
pub trait CsrfExt {
    ...
    // PG_CHANGED
    /// Verify that an out-of-band token value (e.g. HTTP header) matches the
    /// stored CSRF token and refresh its expiration.
    fn verify_token_value<C>(self, clock: &C, token_value: &str)
...
impl CsrfExt for CookieJar {
    ...
    // PG_CHANGED
    fn verify_token_value<C>(self, clock: &C, token_value: &str) -> Result<Self, CsrfError>
    where
        Self: Sized,
        C: Clock,
    {
        let token: CsrfToken = self.load("csrf")?.ok_or(CsrfError::Missing)?;
        let token = token.verify_expiration(clock.now())?;
        token.verify_form_value(token_value)?;
        Ok(self.save("csrf", &token, false))
    }
}
